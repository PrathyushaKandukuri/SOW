...
namespace CandidateSoW.Controllers
{
    ...
    public class SowController : ControllerBase
    {
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        foreach (DataRow dr in ds.Tables[0].Rows)
                        {
                            list.Add(new Sow
                            {
                                ...
                                RoleDetails = dr["RoleDetails"].ToString(),
                                experienceinyears = Convert.ToInt32(dr["experienceinyears"])
        ...
        [HttpPost]
        [Route("SOWAddDataTODB")]
        public string ADDDataTODB([FromBody] Sowdata json_data)
        {
            string dbConn = configuration.GetSection("ConnectionStrings").GetSection("DbConnection").Value;
            Db dop = new Db(dbConn);
            string msg = string.Empty;
        ...
        [HttpGet("SoCandidate")]
        public List<Sow> GetForSoCAndidateMapping()
        {
            string dbConn = configuration.GetSection("ConnectionStrings").GetSection("DbConnection").Value;
            Db dop = new Db(dbConn);
            Sow sp = new Sow();
            sp.Type = "getforSoCandidate";
            DataSet ds = dop.sowtableget(sp, msg);
            List<Sow> list = new List<Sow>();
            try
            {
        ...
        //[HttpPost]
        //[Route("SOWAddDataTODB")]
        //public string ADDDataTODB([FromBody] BillingData json_data)
        //{
        //    string dbConn = configuration.GetSection("ConnectionStrings").GetSection("DbConnection").Value;
        //    Db dop = new Db(dbConn);
        //    string msg = string.Empty;
        //    try
        //    {
        //        string data = JsonConvert.SerializeObject(json_data.Billing);
        //        msg = dop.Add_sowData_DB(data);
        //    }
        //    catch (Exception ex)
        //    {
