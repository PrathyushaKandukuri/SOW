...
namespace CandidateSoW.Controllers
{
    ...
    public class LoginController : ControllerBase
    {
        ...
        public string LoginGet(string EmailId, string LoginPassword)
        {
            ...
            try
            {
                ...
                if (ds.Tables[0].Rows.Count>0)
                {
                    ...
                    ln.TabNames = ds.Tables[0].Rows[0]["TabNames"].ToString();
                    ...
                    ln.IsFirstLogin = (bool)ds.Tables[0].Rows[0]["IsFirstLogin"];
                    ln.Image = ds.Tables[0].Rows[0]["Image"].ToString();
                    ln.MobileNo = ds.Tables[0].Rows[0]["MobileNo"].ToString();
        ...
        [HttpPut]
        public string putprofile(string EmailId, LoginModel lm)
        {
            string dbConn = configuration.GetSection("ConnectionStrings").GetSection("DbConnection").Value;
                Db dop = new Db(dbConn);
            string msg = string.Empty;
            try
            {
                msg = dop.updateProfile(EmailId, lm);
            }
            catch (Exception ex)
            {
                msg = ex.Message;
